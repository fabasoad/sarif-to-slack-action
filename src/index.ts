...
export async function run() {
  const client: SarifToSlackClient = await SarifToSlackClient.create({
    ...
    sarif: {
      ...
      recursive: getBooleanInput('sarif-recursive', { required: true, trimWhitespace: true }),
      extension: processSarifExtension(getInput('sarif-file-extension', { required: true, trimWhitespace: true })),
    },
    log: {
      ...
      colored: getBooleanInput('log-colored', { required: false, trimWhitespace: false }),
    },
    header: {
